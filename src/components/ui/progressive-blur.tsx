...
export function ProgressiveBlur({
  ...
  // Mask bantları eleman sayısına göre bölünür; orijinal bileşen 12.5'i
  // (8 elemanlı varsayılan dizi) hard-code ettiğinden daha kısa özel
  // dizilerde arada blursuz şerit kalıyordu.
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          maskImage:
            position === "bottom"
              ? `linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) ${step}%, rgba(0,0,0,1) ${2 * step}%, rgba(0,0,0,0) ${3 * step}%)`
              : position === "top"
                ? `linear-gradient(to top, rgba(0,0,0,0) 0%, rgba(0,0,0,1) ${step}%, rgba(0,0,0,1) ${2 * step}%, rgba(0,0,0,0) ${3 * step}%)`
      ...
      <div
        ...
        style={{
          ...
          maskImage:
            position === "bottom"
              ? `linear-gradient(to bottom, rgba(0,0,0,0) ${(blurLevels.length - 1) * step}%, rgba(0,0,0,1) 100%)`
              : position === "top"
                ? `linear-gradient(to top, rgba(0,0,0,0) ${(blurLevels.length - 1) * step}%, rgba(0,0,0,1) 100%)`
                : `linear-gradient(rgba(0,0,0,0) 0%, rgba(0,0,0,1) 5%, rgba(0,0,0,1) 95%, rgba(0,0,0,0) 100%)`,
          WebkitMaskImage:
            position === "bottom"
              ? `linear-gradient(to bottom, rgba(0,0,0,0) ${(blurLevels.length - 1) * step}%, rgba(0,0,0,1) 100%)`
