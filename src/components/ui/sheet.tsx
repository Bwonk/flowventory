  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        // Dialog'la aynı ritim: 300ms ease-out giriş, 200ms sessiz çıkış;
        // yüzer katman olduğu için shadow serbest (DESIGN.md elevation kuralı).
        className={cn(
          "bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:duration-300 data-[state=closed]:duration-200 ease-out motion-reduce:data-[state=open]:animate-none motion-reduce:data-[state=closed]:animate-none fixed z-50 flex flex-col gap-4 shadow-lg",
          // Yüzer panel dili (sidebar hap motifi): kenarlara yapışmaz, kendi
          // radius'u ve hairline çerçevesiyle kanvas üzerinde süzülür.
          side === "right" &&
            "data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right inset-y-2 right-2 w-3/4 rounded-lg border border-hairline sm:max-w-sm",
          side === "left" &&
            "data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left inset-y-2 left-2 w-3/4 rounded-lg border border-hairline sm:max-w-sm",
          side === "top" &&
